...
**Five of the model's 17 queries are broken, and three more run perfectly while returning the
wrong number.**
...
catches that, because there's nothing to catch.
...
game?" — 52 different performances are tied for the record, because that statistic is capped in
...
I wrote 24 automated tests where **the expected answers come from queries I wrote myself**, never
from the system's own output — otherwise you just lock in whatever it currently does, including
the mistakes. Each test records the query that proves its number, so anyone can re-check it.

They run automatically on every change, and one step deliberately breaks a test to confirm the
...
- **A number I'd overstated.** I claimed player scores reconcile perfectly across all games. They
  reconcile across 136 of 138 — one game has scores and no player records. I'd quietly redefined
...
```
dotnet run --project src/SportsQa.EvalRunner     # 24 tests, all passing
