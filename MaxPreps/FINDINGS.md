...
**Two silent entity/scope guesses that execute cleanly.** "How many points did Jackson score?"
resolves to Jackson Prep *football* with no acknowledgement that Jackson Prep basketball, the
city of Jackson, and Tony Jackson all match — same class as the rollup: plausible SQL, plausible
number, wrong question answered. And the head-to-head query has **no sport filter at all**,
which is a third defect in that one query alongside the broken quote and the one-sided join.

**Derek Foss's passing yards has no season or sport scope.** Correct today because he appears in
one season; latent in the same way as the top-5 query.
...
**5. Decide the clipped-stat policy.** A 52-way tie probably deserves `cannot_answer` with an
